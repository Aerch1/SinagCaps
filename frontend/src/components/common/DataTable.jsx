...
import {
    Filter, Search, Maximize2, MoreHorizontal, ChevronsUpDown,
...
export default function DataTable({
    ...
    const filtered = useMemo(() => {
        ...
        if (sort.key === "date") {
            data = [...data].sort((a, b) =>
                sort.dir === "asc" ? toDateObj(a) - toDateObj(b) : toDateObj(b) - toDateObj(a)
            );
    ...
    return (
        // MATCH ServiceArea card:
        // rounded-lg, border-gray-200, bg-white, dark:border-slate-700, dark:bg-slate-800
        <div className="h-full rounded-lg border border-gray-200 bg-white p-4 md:p-6 dark:border-slate-700 dark:bg-slate-800">
                ...
                <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" className="h-9 gap-2 shrink-0 dark:border-slate-600 dark:text-slate-100">
                                <Filter className="h-4 w-4" />
                                <span className="hidden sm:inline">Filter</span>
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-56 dark:bg-slate-900 dark:border-slate-700">
                            <DropdownMenuLabel className="dark:text-slate-200">Service Type</DropdownMenuLabel>
                            ...
                            <DropdownMenuLabel className="dark:text-slate-200">Status</DropdownMenuLabel>
                            {statusOptions.length ? (
                                ...
                                <div className="px-2 py-1.5 text-xs text-muted-foreground">No statuses</div>
                            )}
                            <DropdownMenuSeparator className="dark:bg-slate-700" />
                            <DropdownMenuItem onClick={clearFilters} className="dark:text-slate-200 dark:focus:bg-slate-800">
                                Clear filters
                            </DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                    ...
                    <div className="relative order-last w-full min-w-0 sm:order-none sm:w-56">
                        ...
                        <input
                            ...
                            className="h-9 w-full rounded-md border border-input bg-background pl-9 pr-3 text-sm outline-none ring-offset-background focus-visible:ring-2 focus-visible:ring-ring/30 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-100 placeholder:text-slate-400"
                        />
                    </div>
                    ...
                    <Link to={manageHref} className="inline-flex shrink-0">
                        <Button
                            variant="outline"
                            className="h-9 w-9 p-0 sm:w-auto sm:px-3 dark:border-slate-600 dark:text-slate-100"
            ...
            <div className="overflow-x-auto">
                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableHead className={`${cellPad} min-w-[200px] text-gray-500 dark:text-slate-400`}>Client Name · Mail</TableHead>
                            <TableHead className={`${cellPad} text-gray-500 dark:text-slate-400`}>Service Type</TableHead>
                            <TableHead className={`${cellPad} text-gray-500 dark:text-slate-400`}>Status</TableHead>
                            <TableHead
                                className={`${cellPad} cursor-pointer select-none text-gray-500 dark:text-slate-400`}
                            ...
                            <TableHead className={`${cellPad} text-right text-gray-500 dark:text-slate-400`}>Actions</TableHead>
                        </TableRow>
                    </TableHeader>
                    ...
                    <TableBody>
                        {paged.length === 0 ? (
                            <TableRow>
                                <TableCell colSpan={6} className={`${cellPad} py-10 text-center text-sm text-muted-foreground`}>
                                    No results
                                </TableCell>
                            </TableRow>
                        ) : (
                                ...
                                >
                                    ...
                                    <TableCell className={`${cellPad} text-right`}>
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-40 dark:bg-slate-900 dark:border-slate-700">
                                                <DropdownMenuItem onClick={() => onView?.(r)} className="dark:text-slate-200 dark:focus:bg-slate-800">View details</DropdownMenuItem>
                                                <DropdownMenuItem onClick={() => onEdit?.(r)} className="dark:text-slate-200 dark:focus:bg-slate-800">Edit</DropdownMenuItem>
                                                <DropdownMenuSeparator className="dark:bg-slate-700" />
                                                <DropdownMenuItem
            ...
            <div className="mt-4 flex flex-col items-center justify-between gap-3 sm:flex-row">
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" className="h-8 px-3 dark:border-slate-600 dark:text-slate-100">{pageSize}</Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="start" className="dark:bg-slate-900 dark:border-slate-700">
                            {[5, 10, 20].map((n) => (
                                <DropdownMenuItem
                                    key={n}
                ...
                <div className="flex items-center gap-1">
                    <Button
                        variant="outline"
                        className="h-8 px-3 dark:border-slate-600 dark:text-slate-100"
                        disabled={safePage <= 1}
                    ...
                    {pageList(safePage, totalPages).map((n, i, arr) => {
                        ...
                        return (
                            <React.Fragment key={n}>
                                {needsDots && <span className="px-1 text-muted-foreground">…</span>}
                                <Button
                    ...
                    ${isActive
                                            ...
                                            : "text-slate-700 border-slate-300 hover:bg-gray-100 dark:text-slate-300 dark:border-slate-600 dark:hover:bg-slate-700/40"
                                        }`}
                                    onClick={() => setPage(n)}
                                >
                                    {n}
